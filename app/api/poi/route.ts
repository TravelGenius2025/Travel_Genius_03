...
export async function GET(req: Request){
  ...
  const radius = 7000
  const geo = await wiki(`https://en.wikipedia.org/w/api.php?action=query&list=geosearch&gscoord=${latNum}%7C${lonNum}&gsradius=${radius}&gslimit=12&format=json`)
  ...
  const results = Object.values(detail?.query?.pages || {}).map((p:any)=> ({
    ...
    retailPrice: { amount: 0, currency: 'INR' }, // info-only
    viatorUrl: null
  }))
