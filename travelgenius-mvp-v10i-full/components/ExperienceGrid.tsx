...
export default function ExperienceGrid({ cityId }:{ cityId:string }){
  const [items,setItems]=require('react').useState<any[]>([])
  require('react').useEffect(()=>{ fetch(`/api/experiences?cityId=${encodeURIComponent(cityId)}`).then(r=>r.json()).then(j=>setItems(j.data||[])) },[cityId])
