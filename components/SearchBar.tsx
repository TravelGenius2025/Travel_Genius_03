...
export default function SearchBar({ onPick }:{ onPick:(name:string)=>void }){
  ...
  return (<div className='relative'>
    ...
    {sug.length>0 && <div className='absolute z-10 bg-white border rounded-xl mt-1 w-full shadow-soft max-h-64 overflow-auto'>
      {sug.map((c:any)=>(<div key={c.id} onClick={()=>onPick(c.city)} className='px-3 py-2 hover:bg-gray-50 cursor-pointer'>{c.city}{c.countryCode?`, ${c.countryCode}`:''}</div>))}
    </div>}
  </div>)
}
